...
// It returns the session expiration time.
func login(db *sql.DB, email, password string) (expiresAt time.Time, err error) {
	var userID, verifyID int
	var storedPassword string

	// Query the database to get the user's ID, hashed password, and verification status
	err = db.QueryRow("SELECT id, password, verifyid FROM USER WHERE email = ?", email).Scan(&userID, &storedPassword, &verifyID)
	if err != nil {
	...
	err = db.QueryRow("SELECT expires FROM SESSION WHERE userid = ? AND expires > ?", userID, time.Now()).Scan(&existingExpiration)
	if err == nil {
		return existingExpiration, nil
	}
	...
	_, err = db.Exec("INSERT INTO SESSION (userid, expires) VALUES (?, ?)", userID, expiresAt)
	if err != nil {
		return time.Time{}, err
	}
